...
def create_executive_summary(rules: pd.DataFrame, data: pd.DataFrame, insights: Dict) -> str:
    ...
    high_confidence_rules = int(((rules['confidence'] >= 0.8).sum()) if (rules is not None and 'confidence' in rules.columns) else 0)
...
def create_advanced_visualizations(rules: pd.DataFrame) -> None:
    ...
    with tab1:
        ...
        with col2:
            ...
            """.format(
                int(((rules['confidence'] >= 0.8).sum()) if (rules is not None and 'confidence' in rules.columns) else 0),
                int(((rules['lift'] > 2.0).sum()) if (rules is not None and 'lift' in rules.columns) else 0),
                int(((rules['support'] > 0.05).sum()) if (rules is not None and 'support' in rules.columns) else 0)
            ), unsafe_allow_html=True)
    ...
    with tab3:
        ...
        fig_scatter.update_layout(xaxis=dict(tickformat='.1%'))
        fig_scatter.update_layout(xaxis=dict(tickformat='.1%'))
...
def show_action_items():
    ...
    for i, rec in enumerate(recommendations, 1):
        ...
        <div class="pro-container" style="border-left: 4px solid {border_color};">
            ...
            {f'<div style="background: #f3f4f6; padding: 12px; font-size: 0.95rem; border-radius:6px;"><strong>Notes:</strong> {rec["notes"]}</div>' if rec.get("notes") else ""}
        </div>
        """, unsafe_allow_html=True)
