...
# =============================================================================
# Part 2: ENHANCED NETWORK VISUALIZATION AND DATA PROCESSING
# =============================================================================
...
# =============================================================================
# DATA BINNING AND PROCESSING FUNCTIONS
# =============================================================================

def create_professional_bins(df, strategy="business_rules"):
    """Create professional bins for continuous variables with business logic."""
    binned = df.copy()
    ...
    if 'aht' in df.columns:
        if strategy == "business_rules":
            ...
            binned['aht_category'] = pd.qcut(
                df['aht'],
                q=4,
                labels=["AHT_Q1", "AHT_Q2", "AHT_Q3", "AHT_Q4"],
                duplicates='drop'
            )
    ...
    if 'csat' in df.columns:
        binned['csat_category'] = pd.cut(
            ...
            labels=["CSAT_BelowExpectations", "CSAT_NeedsImprovement", 
                   "CSAT_MeetsExpectations", "CSAT_ExceedsExpectations"],
            include_lowest=True
        )
    ...
    if 'adherence' in df.columns:
        binned['adherence_category'] = pd.cut(
            ...
            labels=["Adherence_Concerning", "Adherence_BelowTarget", 
                   "Adherence_OnTarget", "Adherence_Excellent"],
            include_lowest=True
        )
    ...
    if 'qa_score' in df.columns:
        binned['qa_category'] = pd.cut(
            ...
            labels=["QA_NeedsImprovement", "QA_BelowStandard", 
                   "QA_MeetsStandard", "QA_Exceptional"],
            include_lowest=True
        )
    ...
    if 'escalations' in df.columns:
        binned['escalation_category'] = pd.cut(
            ...
            labels=["Escalations_None", "Escalations_Minimal", 
                   "Escalations_Moderate", "Escalations_High"],
            include_lowest=True
        )
    
    # ACW binning
    if 'acw' in df.columns:
        binned['acw_category'] = pd.cut(
            df['acw'],
            bins=[0, 30, 60, 120, float('inf')],
            labels=["ACW_Minimal", "ACW_Standard", "ACW_Extended", "ACW_Excessive"],
            include_lowest=True
        )
    
    # Hold time binning
    if 'hold_time' in df.columns:
        binned['hold_category'] = pd.cut(
            df['hold_time'],
            bins=[0, 30, 60, 120, float('inf')],
            labels=["Hold_Minimal", "Hold_Acceptable", "Hold_Extended", "Hold_Excessive"],
            include_lowest=True
        )
    
    # Repeat contacts binning
    if 'repeat_contacts' in df.columns:
        binned['repeat_category'] = pd.cut(
            df['repeat_contacts'],
            bins=[-0.1, 0, 1, 3, float('inf')],
            labels=["Repeats_None", "Repeats_Low", "Repeats_Moderate", "Repeats_High"],
            include_lowest=True
        )
    
    # Absenteeism binning
    if 'absenteeism' in df.columns:
        binned['absence_category'] = pd.cut(
            df['absenteeism'],
            bins=[0, 0.02, 0.05, 0.1, float('inf')],
            labels=["Absence_Excellent", "Absence_Good", "Absence_Concerning", "Absence_Critical"],
            include_lowest=True
        )
    
    # Add call type if present
    if 'call_type' in df.columns:
        binned['calltype_category'] = 'CallType_' + df['call_type'].astype(str)
...
def create_professional_bins_enhanced(df, strategy="business_rules"):
    ...
    if strategy == "auto_adaptive":
        return create_adaptive_bins(df)
    else:
...
def create_market_basket_enhanced(df_binned):
    ...
    # Find columns that are categorical or end with _bin, _category
    bin_cols = [c for c in df_binned.columns if c.endswith("_bin") or c.endswith("_category")]
    ...
    for col in bin_cols:
        try:
            feature_name = col.replace("_bin", "").replace("_category", "").replace("_", "")
    ...
    if basket_parts:
        ...
        return pd.DataFrame()
        # =============================================================================
# Part 3: ASSOCIATION RULE MINING AND ANALYSIS FUNCTIONS
# =============================================================================
...
def clear_analysis_results():
    ...
    keys_to_clear = [
        "association_rules", "business_insights", "network_html", "analysis_timestamp"
    ]
    for key in keys_to_clear:
        if key in st.session_state:
            del st.session_state[key]
...
# =============================================================================
# Part 5: DATA OVERVIEW AND ANALYSIS SETUP FUNCTIONS
# =============================================================================
        ...
        with col1a:
            if st.button("🎲 Generate Sample Data", type="primary", use_container_width=True):
                with st.spinner("Generating sample dataset..."):
                    st.session_state.raw_data = generate_professional_sample_data_enhanced()
...
# =============================================================================
# Part 6: SAMPLE DATA GENERATION AND UTILITY FUNCTIONS
# =============================================================================
...
# =============================================================================
# Part 7: RESULTS DISPLAY AND VISUALIZATION FUNCTIONS
# =============================================================================
...
# =============================================================================
# Part 8: EXPORT, REPORT GENERATION AND CHART CREATION FUNCTIONS
# =============================================================================
...
# =============================================================================
# Part 9: MAIN APPLICATION AND COMPLETE PACKAGE FUNCTIONS
# =============================================================================
...
def create_enhanced_complete_package(rules, data, insights):
    ...
    with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
        ...
        rules_parquet = io.BytesIO()
        rules.to_parquet(rules_parquet, index=False)
        zf.writestr("data/association_rules.parquet", rules_parquet.getvalue())
        
        if st.session_state.get('binned_data') is not None:
            binned_data = st.session_state.binned_data
            zf.writestr("data/processed_data.csv", binned_data.to_csv(index=False))
            
            binned_parquet = io.BytesIO()
            binned_data.to_parquet(binned_parquet, index=False)
            zf.writestr("data/processed_data.parquet", binned_parquet.getvalue())
        
        zf.writestr("data/original_data.csv", data.to_csv(index=False))
        
        data_parquet = io.BytesIO()
        data.to_parquet(data_parquet, index=False)
        zf.writestr("data/original_data.parquet", data_parquet.getvalue())
        ...
        metadata = {
            ...
            "pattern_analytics": {
                ...
                "risk_distribution": rules['risk_category'].value_counts().to_dict() if 'risk_category' in rules.columns else {},
                "confidence_stats": {
            ...
            "performance_metrics": {
                ...
                "total_coverage": float(rules['support'].sum()) if not rules.empty else 0,
                "pattern_diversity": len(rules['risk_category'].unique()) if 'risk_category' in rules.columns else 0
            }
        }
...
## Overview
Advanced pattern recognition platform for contact center performance optimization with machine learning-powered insights.
...
### `/visualizations/`
- Interactive charts and network visualizations
...
### Business Impact
- **Coverage:** {rules['support'].sum():.1%} of all operations
...
3. **Enhanced Network Visualization** - Interactive, filterable pattern networks
4. **Advanced Business Intelligence** - Risk assessment, opportunities, insights
5. **Fixed Action Items Section** - No HTML rendering issues, proper Streamlit components
6. **Comprehensive Export Options** - CSV, Parquet, HTML, and complete packages
